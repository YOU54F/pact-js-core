...
if [ -z "${ONLY_DOWNLOAD_PACT_FOR_WINDOWS:-}" ]; then
  ...
  download_standalone "pact-${STANDALONE_VERSION}-osx-arm64.tar.gz"           "darwin-aarch64-${STANDALONE_VERSION}.tar.gz"
  download_standalone "pact-${STANDALONE_VERSION}-linux-x86_64.tar.gz"  "linux-x64-${STANDALONE_VERSION}.tar.gz"
  download_standalone "pact-${STANDALONE_VERSION}-linux-arm64.tar.gz"  "linux-aarch64-${STANDALONE_VERSION}.tar.gz"
fi
