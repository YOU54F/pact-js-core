...
## Also raise a PR against pact-node-arm64

git checkout pact-node-arm64
git checkout standalone/install.ts
git pull origin pact-node-arm64

git checkout -b ${BRANCH_NAME}-pact-node-arm64
...
git add standalone/install.ts
git commit -m "${TYPE}: update standalone to ${STANDALONE_VERSION}"
git push --set-upstream origin ${BRANCH_NAME}-pact-node-arm64
